...
Additionally, let's remove the tasks variable from the **TaskService** class as we no longer need it: the data will be fetched from the API server.
...
1. Update the `src/app/task.service.ts` file.
    ...
    ```typescript del={"Remove the task variable": 9-21} ins={"Add the getTasks function": 24-27}
...
1. Update the the `src/app/task-list/task-list.component.html` file.
    ...
    ```html ins={"Update the HTML template": 1-2} ins="tasks$ | async"
    <ul>
