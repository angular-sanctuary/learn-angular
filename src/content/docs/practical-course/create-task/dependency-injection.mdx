...
:::caution
As we are deleting this list, the application will stop working for a moment as the html file is still expecting a `tasks` variable.
:::
...
To do so, we reference the `TaskService` class in the `TaskListComponent` class constructor.
