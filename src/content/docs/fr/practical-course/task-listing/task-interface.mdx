...
Nous allons créer une nouvelle interface TypeScript pour représenter une tâche.
...
Nous générerons un identifiant unique pour chaque tâche. Cet identifiant nous aidera à trouver une tâche donnée de manière unique.
Un tel identifiant est généralement généré par un serveur backend, mais pour ce tutoriel, nous le générerons côté client.

Pour ce faire, nous utiliserons la libairie `uuid`. Cette libairie génère des identifiants uniques basés sur l'horodatage actuel et un nombre aléatoire.
...
Créons une variable locale dans la classe `TaskListComponent` pour stocker une liste de tâches.
Nous utiliserons l'interface `Task` pour définir le type de la variable tâches.
...
Quel est le mot-clé TypeScript utilisé pour définir le modèle de tâche dans le fichier `src/app/model/task.model.ts` ?

1. `interface`
2. `type`
3. `class`

Quel est le mot-clé TypeScript utilisé pour définir le type d'une variable ?
