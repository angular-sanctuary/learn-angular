...
1. Modifiez le fichier `src/app/task.service.ts`.
    ...
    ```typescript ins={"Ajoutez la fonction addTask": 10-16}
...
1. Modifiez le fichier `src/app/task-form/task-form.component.ts`.
    ...
    ```typescript ins={"Mettez à jour la fonction de soumission du formulaire": 19-23} ins="this.taskService.addTask(task).subscribe();"
...
1. Modifiez le fichier `src/app/task-form/task-form.component.ts`.
    ...
    ```typescript ins={"Mettez à jour la fonction de soumission du formulaire": 20-25}
    import { Component } from '@angular/core';
