...
Vous allez supprimer la variable tâches de la classe **TaskService** car vous n'en aurez plus besoin. Effectivement, les données seront récupérées depuis l'API.
...
1. Mettez à jour le fichier `src/app/task.service.ts`.
    ...
    ```typescript del={"supprimer la variable task": 9-21} ins={"Ajoutez la fonction getTasks": 24-27}
...
1. Modifiez le fichier `src/app/task-list/task-list.component.html`.
    ...
    ```html ins={"Modifiez le Template HTML": 1-2} ins="tasks$ | async"
    <ul>
