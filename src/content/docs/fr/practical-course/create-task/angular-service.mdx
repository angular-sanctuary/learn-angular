...
La commande `ng generate` que vous avez utilisée pour les composants peut également être utilisée pour générer des services.
...
2. Ouvrez le fichier `task.service.ts` et ajoutez la  une propriété pour stocker la liste des tâches:

```typescript ins={"Importez l\'interface des tâches": 3-4} ins={"Ajoutez la variable des tâches": 10-24}
// task.service.ts
