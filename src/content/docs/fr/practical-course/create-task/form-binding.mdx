...
description: Apprenez à lier un formulaire à un modèle dans Angular
---

import Objective from '../../../../../components/learning-objective.astro';

<Objective class="drop-shadow-xl" objectives={[
{mainTitle: 'Objectifs de ce chapitre', title: 'Liaison de votre formulaire HTML', description: 'Apprenez à lier un formulaire à un modèle (`.component.ts`) dans Angular.'},
]} />
...
- Formulaires pilotés par le Template HTML;
- Formulaires réactifs piloté par le code (`.component.ts`).

Dans ce cours, vous utiliserez l'API des formulaires pilotés par le Template HTML pour lier le formulaire.

## Formulaire piloté par le Template HTML
...
Vous allez créer un modèle de `task` pour représenter une tâche. Le modèle `task` aura deux propriétés : `title` et `description`.
...
Le type d'utilitaire `Pick` vous permet de créer un nouveau type en sélectionnant certaines propriétés d'un type existant.
...
```typescript ins={"Importez le nouveau type TaskForm": 3-4} ins={"Ajoutez le modèle de tâche avec des valeurs vides": 12-16}
...
Pour lier un formulaire à un modèle (`.component.ts`) dans Angular, vous pouvez utiliser la directive `[ngModel]`.
La directive `[ngModel]` est une directive intégrée qui vous permet de lier une entrée de formulaire à un modèle.
Mais cela ne suffira pas pour votre cas d'utilisation : cela vous permet uniquement de lier la valeur de la propriété au champ, et non l'inverse.

### Liaison de données bidirectionnelle

Pour mettre à jour votre modèle `task` lorsque le formulaire est mis à jour, vous devez utiliser le format `[(ngModel)]`.
...
En étant lié au formulaire, le modèle `task` sera mis à jour lorsque le formulaire sera mis à jour.

## ✔️ Ce que vous avez appris

Dans ce chapitre, vous avez appris à lier un formulaire à un modèle (`.component.ts`) dans Angular. Vous avez appris à créer un modèle `task` pour représenter une tâche. Vous avez également appris à utiliser la directive `[(ngModel)]` pour lier une entrée de formulaire à un modèle.
